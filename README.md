...
- **Jira / Linear tasks** — by default, a task branch is created from the story branch (story branch is ensured first from `base_branch` if missing). For smaller stories you can choose to work directly on the **story branch** instead; the choice is stored in `.git/config` per story branch (see [Branching mode](#branching-mode))
- **New branches** — created with `--no-track`, then pushed with `git push -u origin <branch>` so upstream is `origin/<branch>`, not master. The parent branch is recorded as `vscode-merge-base` in `.git/config` (e.g. `origin/master` for a story branch, `origin/feature/PROJ-123-slug` for a task branch)
...
**Update mode** (`--update`): when the working tree is dirty, prompts to stash before updating (restored after). Matches the current git branch to an assigned in-progress issue when possible and uses that issue’s parent for the upmerge; otherwise updates the current branch from `base_branch` (even when no issues are listed). No issue picker. When the current branch is a story branch, the matched **Story** is shown (even if only Subtaak/sub-issues are assigned). Fetches origin, merges `origin/<current-branch>` when it exists, then upmerges the parent:

- **Matched story / GitHub issue** — parent is `base_branch`
- **Matched Jira / Linear task (task branch mode)** — parent is the story branch
- **Matched Jira / Linear task (story branch mode)** — parent is `base_branch`
- **No match / empty issue list** — uses stored `vscode-merge-base` from branch creation when present, otherwise `base_branch`
...
For Jira Subtaak/Taak and Linear sub-issues, the wizard asks once per story whether to use a **task branch** (separate branch per task) or the **story branch** (all tasks commit on the same branch). The decision is saved in local git config:

```ini
[branch "feature/PROJ-123-add-pest-testing"]
    branch-helper-mode = story
```

- Values: `story` or `task` (default when prompted: task)
- Keyed by **story branch name**, so all tasks in the same story share one mode
- On later runs, the stored mode is applied automatically (shown as `branching mode: … (stored)`)
- Commits always use the **task** subject `(TASK-KEY) title`, even when checked out on the story branch
- To change or reset: `git config --unset branch."feature/PROJ-123-slug".branch-helper-mode`

### Jira output

For each issue it shows:

- **branch** — story branch name. For Stories and other non-task issues: the **selected issue's** key + slugified summary (not a parent Feature/Epic). For Subtaak/Taak: the **parent story's** key + summary. Bug issues use `bug/{key}-{slug}` (e.g. `bug/PROJ-123-fix-login-error`); other types use `feature/{key}-{slug}` (e.g. `feature/PROJ-123-add-pest-testing`)
- **task branch** — task branch name (parent + task key + lowercase slugified summary), for Subtaak/Taak issues. Uses the same `bug/` or `feature/` prefix as the parent bug or story (e.g. `feature/PROJ-123-PROJ-456-add-tests`, `bug/PROJ-123-PROJ-456-fix-regression`)
...
- **branch** — story branch name (`feature/{identifier}-{slugified-title}`). For top-level issues: the **selected issue's** identifier + title. For sub-issues: the **parent issue's** identifier + title (e.g. `feature/ENG-123-add-auth-flow`)
